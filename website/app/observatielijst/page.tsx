...
function Lijn({ label }: { label?: string }) {
  return (
    <label className="block">
      {label ? (
        <span className="text-[13px] font-bold text-ink-dim">{label}</span>
      ) : null}
      <input
        type="text"
        aria-label={label}
        className="mt-0.5 block h-8 w-full border-0 border-b border-border bg-transparent text-[15px] text-ink outline-none focus:border-green"
      />
    </label>
  );
}
  ...
  return (
    <>
      ...
      <Sectie className="py-6">
        <p className="rounded-[20px] border-2 border-orange bg-orange-soft px-6 py-5 text-[15px] font-bold text-ink">
      ...
      <Sectie className="niet-afdrukken grid gap-4 py-0 md:grid-cols-2">
        <Kaart>
          ...
          <ul className="mt-3 grid gap-2">
            {observatieTekst.hoeInvullen.map((regel) => (
              <li key={regel} className="flex gap-2.5 text-[15px] text-ink-dim">
        ...
        <Kaart className="self-start">
          ...
          <p className="mt-3 text-[15px] text-ink-dim">{observatieTekst.afdrukken}</p>
        </Kaart>
      </Sectie>
          ...
          <Kaart key={onderdeel.nummer} className="afdruk-kaart min-w-0">
            <div className="flex flex-wrap items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[15px] text-ink-dim">{onderdeel.inleiding}</p>
              </div>
            </div>
            ...
            <div className="mt-5 flex items-end gap-1 border-b border-border pb-1.5 sm:gap-2">
              ...
              {kolommen.map((kolom) => (
                <span
                  key={kolom}
                  className="w-10 text-center text-[11px] leading-tight font-bold text-ink-dim sm:w-14"
                >
                  {kolom}
                </span>
              ))}
              ...
              {onderdeel.vragen.map((vraag) => (
                ...
                >
                  <span className="min-w-0 flex-1 text-[15px] text-ink">
                    {vraag.tekst}
                    {vraag.toelichting ? (
                      <span className="text-ink-dim"> ({vraag.toelichting})</span>
                    ) : null}
                  </span>
                  {kolommen.map((kolom) => (
                    <span
            ...
            >
              ...
              {observatieTekst.plaatsOpties.map((optie) => (
                <label key={optie} className="flex items-center gap-2 text-[14px] text-ink">
      ...
      <Sectie className="py-0">
        <Kaart className="afdruk-kaart">
          ...
          <p className="mt-1 text-[15px] text-ink-dim">
            Deze antwoorden zijn voor een specialist vaak het waardevolste stuk van het blad.
          </p>
          <div className="mt-5 grid gap-6">
            {openVragen.map((open) => (
              <div key={open.vraag}>
                <p className="text-[15px] font-bold text-ink">{open.vraag}</p>
      ...
      <Sectie className="py-10">
        <Kaart className="afdruk-kaart">
          ...
          <p className="mt-3 max-w-[70ch] text-[15px] text-ink-dim">{overlapTekst.inleiding}</p>

          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {overlapTekst.paren.map((paar) => (
              <div key={paar.titel} className="rounded-[16px] bg-cream px-5 py-4">
                <h3 className="text-[17px] text-purple">{paar.titel}</h3>
                <p className="mt-2 text-[15px] text-ink-dim">{paar.tekst}</p>
              </div>
            ))}
          </div>

          <p className="mt-6 rounded-[16px] bg-purple-soft px-5 py-4 text-[15px] font-bold text-ink">
            {overlapTekst.nadruk}
          </p>
        </Kaart>
      </Sectie>
      ...
      <Sectie className="py-0">
        <div className="rounded-[20px] border-2 border-orange bg-surface px-6 py-5">
          ...
          <p className="mt-2 text-[15px] text-ink">{hulpNu.tekst}</p>
        </div>
      </Sectie>
      ...
      <Sectie className="niet-afdrukken pb-16">
        <div className="rounded-[20px] bg-green px-7 py-8 text-cream">
          ...
          <p className="mt-3 max-w-[58ch] text-cream/85">{observatieTekst.slot.tekst}</p>
          <Link

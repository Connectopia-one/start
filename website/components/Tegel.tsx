...
export function Tegel({
  ...
  return (
    ...
    >
      {onderdeel.label ? (
        <span className="self-start rounded-full bg-surface px-3 py-1 text-[13px] font-extrabold text-ink-dim">
          {onderdeel.label}
        </span>
      ) : null}
      ...
      <h3 className={`text-xl ${tekstKleur[onderdeel.kleur]}`}>
        {onderdeel.titel}
      </h3>
      <p className="text-[16px] text-ink-dim">{onderdeel.omschrijving}</p>

      {children}

      <span
        className={`mt-auto pt-3 text-[15px] font-extrabold ${tekstKleur[onderdeel.kleur]}`}
      >
        Bekijken →
      </span>
    </NaarLink>
